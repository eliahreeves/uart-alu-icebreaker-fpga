...
export UART_STL_DIR := $(abspath third_party/alexforencich_uart)
export YOSYS_DATDIR := $(shell yosys-config --datdir)

RTL := $(shell \
 BASEJUMP_STL_DIR=$(BASEJUMP_STL_DIR) \
 UART_STL_DIR=$(UART_STL_DIR) \
...
icestorm_icebreaker_gls: synth/icestorm_icebreaker/build/synth.v
	verilator --threads 8 lint/verilator.vlt --Mdir ${TOP}_$@_dir -f synth/icestorm_icebreaker/gls.f -f dv/dv.f  --binary -Wno-fatal --top ${TOP}
	./${TOP}_$@_dir/V${TOP} +verilator+rand+reset+2
